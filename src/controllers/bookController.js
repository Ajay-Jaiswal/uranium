...
const getBooksData= async function (req, res) {
     ...
     //({$or : [ {authorName : "Jamos Goslin" } , { isPublished: true } , {  "year": 1972 }]})// COUNT
...
const bookList = async function (req, res) {
    ...
    let bookList= await BookModel.find().select({authorName : 1 , bookName: 1 , _id: 0}) 
...
const getBooksInYear = async function (req, res) {
    ...
    let BooksInYear= await BookModel.find( {  year: {$eq: 1972}  } ) 
...
const getXINRBooks = async function (req, res) {
    ...
    let XINRBooks= await BookModel.find( {  year: {$in:[ 1972,1910]}  } ) 
...
const getRandomBooks = async function (req, res) {
    ...
    let getRandomBooks= await BookModel.find( {  pages: {$gt: 500}  } ) 
